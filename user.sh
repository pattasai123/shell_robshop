...
curl -o /tmp/user.zip https://roboshop-artifacts.s3.amazonaws.com/user-v3.zip &>> $filename

cd /app 
rm -rf /app/*
unzip /tmp/user.zip &>> $filename
...
systemctl enable user &>> $filename
validate $? "enable user"

systemctl restart user &>> $filename
validate $? "start user"